...
arpid() {
MACS=$(arptable | awk -F "\t" '{print $2}' | egrep -v "incomplete" | awk -F ":" '{if (length($1) == 1) {a = 0$1}  else {a = $1} if (length($2) == 1) {b =  0$2} else {b = $2} if (length($3) == 1) {c = 0$3} else {c = $3} print a,b,c }' | tr -d " "  | tr "\n" ' ' )
